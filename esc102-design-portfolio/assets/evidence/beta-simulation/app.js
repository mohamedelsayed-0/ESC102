(function () {
  ...
  function departureCopy(summary) {
    if (!summary.missedDepartureProbability && !summary.meanDepartureMissSeconds) {
      return "No missed departures in the simulation.";
    }

    return `Missed the actual departure in ${BetaSimulation.round(summary.missedDepartureProbability * 100, 0)}% of simulated days.`;
  }

  function completionCopy(summary) {
    return `End-to-end completion averaged ${BetaSimulation.formatDuration(summary.meanCompletionSeconds)}.`;
  }
    ...
    const cards = [
      {
        ...
        value: `${BetaSimulation.formatDuration(results.parentEmployeeBaselineSeconds)} active staff`,
        body: `${completionCopy(results.baseline)} ${departureCopy(results.baseline)} Task switching is estimated from staffed-line interruptions.`,
      },
      {
        title: "Parent Model",
        value: thresholdCopy(parentBest.thresholdPercent, "parent"),
        body: `Default case: ${BetaSimulation.formatDuration(results.parentDefaultSummary.meanEmployeeSeconds)} active staff. ${completionCopy(results.parentDefaultSummary)} ${departureCopy(results.parentDefaultSummary)} Employee time excludes parent effort outside the classroom.`,
      },
      {
        title: "Machine Model",
        ...
        body: `Default case at 1 machine: ${BetaSimulation.formatDuration(results.machineDefaultRiskByCount.find((item) => item.machineCount === 1).summary.meanEmployeeSeconds)} active staff and ${BetaSimulation.formatDuration(results.machineDefaultRiskByCount.find((item) => item.machineCount === 1).summary.meanCompletionSeconds)} end-to-end. Default case at 2 machines: ${BetaSimulation.formatDuration(results.machineDefaultRiskByCount.find((item) => item.machineCount === 2).summary.meanEmployeeSeconds)} active staff and ${BetaSimulation.formatDuration(results.machineDefaultRiskByCount.find((item) => item.machineCount === 2).summary.meanCompletionSeconds)} end-to-end. 2 machines: ${thresholdCopy(machineTwo.thresholdPercent, "machine")}.`,
      },
    ];
