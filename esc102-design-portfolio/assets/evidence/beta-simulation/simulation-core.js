(function (root, factory) {
  ...
  function createStaffedQueueState(lineCount) {
    return {
      availableAt: Array(lineCount).fill(0),
      busyPeriods: Array(lineCount).fill(0),
      hasStarted: Array(lineCount).fill(false),
    };
  }

  function scheduleStaffedTask(state, lineIndex, readyAt, duration) {
    const previousAvailableAt = state.availableAt[lineIndex];
    const startsNewBusyPeriod =
      !state.hasStarted[lineIndex] || readyAt > previousAvailableAt;

    if (startsNewBusyPeriod) {
      state.busyPeriods[lineIndex] += 1;
    }

    state.hasStarted[lineIndex] = true;

    const serviceStart = Math.max(readyAt, previousAvailableAt);
    const serviceDone = serviceStart + duration;
    state.availableAt[lineIndex] = serviceDone;
  ...
  function countBusyPeriods(state) {
    return sum(state.busyPeriods);
  }
  ...
  function summarizeRuns(runResults, deadlineSeconds, departureSeconds) {
    ...
    const taskSwitches = runResults.map(function collect(run) {
      return run.taskSwitches || 0;
    });
    const supervisionInterruptions = runResults.map(function collect(run) {
      return run.supervisionInterruptions || 0;
    });
    ...
    const departureMissSeconds = completionSeconds.map(function lateness(value) {
      return Math.max(0, value - departureSeconds);
    });
    const departureMissOnly = departureMissSeconds.filter(function keep(value) {
      return value > 0;
    });
    ...
    return {
      ...
      meanTaskSwitches: average(taskSwitches),
      p10TaskSwitches: percentile(taskSwitches, 0.1),
      p90TaskSwitches: percentile(taskSwitches, 0.9),
      meanSupervisionInterruptions: average(supervisionInterruptions),
      p10SupervisionInterruptions: percentile(supervisionInterruptions, 0.1),
      p90SupervisionInterruptions: percentile(supervisionInterruptions, 0.9),
      ...
      meanDepartureMissSeconds: average(departureMissSeconds),
      missedDepartureProbability:
        departureMissOnly.length / Math.max(1, runResults.length),
      meanDepartureMissSecondsIfMissed: average(departureMissOnly),
      p90DepartureMissSeconds: percentile(departureMissOnly, 0.9),
      meanFailureCounts,
    };
  }
  ...
  function collectScenarioRuns(config, seedOffset, runner) {
    const results = [];
    const deadlineSeconds = getReadyDeadlineSeconds(config);
    const departureSeconds = getArrivalWindowSeconds(config);
    ...
    return summarizeRuns(results, deadlineSeconds, departureSeconds);
  }

  function simulateBaseline(config, rng) {
    const children = buildChildren(config, rng);
    const wristbandQueue = createStaffedQueueState(1);
    const classroomQueue = createStaffedQueueState(config.classrooms);
    ...
    for (const child of children) {
      ...
      const wristbandDone = scheduleStaffedTask(
        wristbandQueue,
        0,
        child.arrival,
        wristbandTime,
      ...
      const attendanceDone = scheduleStaffedTask(
        classroomQueue,
        classroomIndex,
        wristbandDone,
        config.baselineAttendanceSeconds,
      ).serviceDone;
    ...
    const supervisionInterruptions = countBusyPeriods(classroomQueue);
    const taskSwitches =
      supervisionInterruptions * 2
      + countBusyPeriods(wristbandQueue) * 2;

    return {
      totalCompletionSeconds,
      totalEmployeeSeconds,
      taskSwitches,
      supervisionInterruptions,
  ...
  function simulateParentScenario(config, rng, complianceRate, incorrectRate) {
    ...
    for (const child of children) {
      ...
      const classroomIndex = child.classroom;
      const serviceDone = scheduleStaffedTask(
        classroomQueue,
        classroomIndex,
        child.arrival,
        employeeSeconds,
    ...
    return {
      totalCompletionSeconds,
      totalEmployeeSeconds,
      taskSwitches: supervisionInterruptions * 2,
      supervisionInterruptions,
      failureCounts,
    };
  }
  ...
  function simulateMachineScenario(
    ...
    for (const child of children) {
      ...
      const employeeSeconds = successful
        ...
        : checkSeconds + config.redoSeconds;
      const classroomIndex = child.classroom;
      const classroomDone = scheduleStaffedTask(
        classroomQueue,
        classroomIndex,
        machineDone,
        employeeSeconds,
