...
function buildSummaryJson() {
  ...
  function operationalMetrics(summary) {
    return {
      activeStaffSeconds: round(summary.meanEmployeeSeconds, 3),
      activeStaffMinutes: durationMinutes(summary.meanEmployeeSeconds),
      meanCompletionSeconds: round(summary.meanCompletionSeconds, 3),
      meanCompletionMinutes: durationMinutes(summary.meanCompletionSeconds),
      taskSwitches: round(summary.meanTaskSwitches, 3),
      taskSwitchesNote:
        "Estimated as two switches for each staffed-line interruption (a line goes idle and later restarts).",
      supervisionInterruptions: round(summary.meanSupervisionInterruptions, 3),
      missedDepartureProbabilityPercent: round(
        summary.missedDepartureProbability * 100,
        3,
      ),
      missedDepartureMetricNote:
        "Based on full end-to-end completion time, including machine queueing and classroom verification.",
      activeStaffMetricNote:
        "Counts employee-handled seconds only; excludes parent effort and machine waiting time.",
    };
  }
  ...
  const payload = {
    ...
    requestedMetrics: {
      currentProcess: operationalMetrics(results.baseline),
      parentDefault: operationalMetrics(results.parentDefaultSummary),
      machineOneDefault: operationalMetrics(machineOneRisk),
      machineTwoDefault: operationalMetrics(machineTwoRisk),
    },
          ...
          <section class="panel">
            ...
            <div class="cards">
              <article class="card">
                ...
                <strong>${formatDuration(results.parentDefaultSummary.meanEmployeeSeconds)} active staff</strong>
                <p class="muted">End-to-end completion ${formatDuration(results.parentDefaultSummary.meanCompletionSeconds)}. ${thresholdLabel(results.parentThresholds[0].thresholdPercent, "parent")}. ${lateText(results.parentDefaultSummary)}</p>
              </article>
              <article class="card">
                <h3>1 machine default</h3>
                <strong>${formatDuration(machineOneRisk.meanEmployeeSeconds)} active staff</strong>
                <p class="muted">End-to-end completion ${formatDuration(machineOneRisk.meanCompletionSeconds)}. ${thresholdLabel(results.machineThresholds.find((item) => item.machineCount === 1).thresholdPercent, "machine")}. ${lateText(machineOneRisk)}</p>
              </article>
              <article class="card">
                <h3>2 machines default</h3>
                <strong>${formatDuration(machineTwoRisk.meanEmployeeSeconds)} active staff</strong>
                <p class="muted">End-to-end completion ${formatDuration(machineTwoRisk.meanCompletionSeconds)}. ${thresholdLabel(results.machineThresholds.find((item) => item.machineCount === 2).thresholdPercent, "machine")}. ${lateText(machineTwoRisk)}</p>
              </article>
            </div>
            <p class="muted">Active staff time excludes parent effort and machine waiting. Missed departure probability uses full end-to-end completion time. Task switching is an estimate: two switches per staffed-line interruption.</p>
          </section>
